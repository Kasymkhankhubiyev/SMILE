...
=============*/
  ...
  R8 Prob_VV12_M[2];
/*
  int incrementL,incrementM;

  if (col->gL->EV[0][Lv_L[0]]>col->gL->EV[1][Lv_L[1]])
  {
    Prob_VV12_L = Prob_VV12_decrease(col->gL,col->gM,Erel,Lv_L[0],Lv_L[1]);
    incrementL = -1;
  }
  else
  {
    Prob_VV12_L = Prob_VV12_increase(col->gL,col->gM,Erel,Lv_L[0],Lv_L[1]);
    incrementL = 1;
  }
  if (col->gM->EV[0][Lv_M[0]]>col->gM->EV[1][Lv_M[1]])
  {
    Prob_VV12_M = Prob_VV12_decrease(col->gM,col->gL,Erel,Lv_M[0],Lv_M[1]);
    incrementM = -1;
  }
  else
  {
    Prob_VV12_M = Prob_VV12_increase(col->gM,col->gL,Erel,Lv_M[0],Lv_M[1]);
    incrementM = 1;
  }
*/
									...
									)
{
 ...
 // if (strcmp(gA->name,"CO2")==0) return 0;
  ...
  //prob = pow(alpha*A_2/(M_PI),4)*pow(A_1/(Masredvib_2*Nu_2),2)*planck*gA->c_massr[gB->id]*Erel/(256*Masredvib_1*Nu_1);
  prob = pow(alpha/(M_PI),6)*pow(A_2,4)*pow(A_1/(Masredvib_2*Nu_2),2)*pow(planck,3)/(2048.*Masredvib_1*Nu_1);
  prob = prob * (lv1+1)*lv2*(lv2-1)*(lv2-1);
  return prob;
}
